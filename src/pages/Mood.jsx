...
const Mood = () => {
  ...
  const getFilteredSongs = () => {
    ...
    // Mapping internal needs to songs.json needs
    const needMapping = {
      // Triste
    ...
    const targetNeed = needMapping[selectedNeed];
    const filtered = songsData.filter(song => 
      normalizeString(song.emotion) === normalizeString(selectedMood) && 
      (targetNeed ? normalizeString(song.need) === normalizeString(targetNeed) : true)
    );

    // If strict filtering returns empty (e.g. need mismatch), fallback to just emotion
    return filtered.length > 0 ? filtered : songsData.filter(song => normalizeString(song.emotion) === normalizeString(selectedMood)).slice(0, 4);
  };

  const filteredSongs = getFilteredSongs();
  const currentPlaylist = selectedMood ? playlistsData[selectedMood] : null;
                ...
                {filteredSongs.map((song) => (
                  ...
                  >
                    <div className="aspect-square w-full overflow-hidden relative bg-white/5">
                      {imgError[song.id] ? (
                        <div className="w-full h-full flex flex-col items-center justify-center text-white/30 p-4 bg-gradient-to-br from-white/5 to-white/10">
                          <motion.span 
                          ...
                          <span className="text-xs text-center font-medium opacity-70">Sin portada</span>
                        </div>
                      ) : (
                        <img 
                          src={song.coverUrl} 
